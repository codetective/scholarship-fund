...
function BirthCertUpload({ doc, app_id, setDocuments }) {
  ...
  const upload = async () => {
    ...
    form.append('image_for', 'birth_certificate_declaration');
    try {
      ...
      setDocuments((prev) => {
        ...
        data.birthCertificate = res.data.url;
        return { ...data };
      });
      closeModal();
      setLoading(false);
  ...
  return (
    <>
      <label htmlFor='birth_certificate_declaration'>
      ...
      <div className='border rounded w-full py-2 px-2 mt-2 mb-5'>
        <input
          ...
          type='file'
          accept='image/*'
          id='birth_certificate_declaration'
          onChange={handleImageSelect}
          className='hidden'
          required
          name='birth_certificate_declaration'
        />

        <label
          htmlFor='birth_certificate_declaration'
...
export default BirthCertUpload;
